...
class Pic_Social_Sharing{
    ...
    private function social_counts($social)
    {
        ...
        $url = wp_nonce_url("http://api.sharedcount.com/?url=" . rawurlencode($permalink),'pic-sharedcount-api');
        ...
        $json = $wp_filesystem->get_contents("http://api.sharedcount.com/?url=" . rawurlencode($url));
    ...
    public function get_social_share_button($social){
        ...
        switch($social){
            ...
            case 'tw';
                ...
                if($this->twitter == 'tw')
                {
                    $output .= '<li><a title="Twitter: '.$this->social_counts('twitter').'" href="https://twitter.com/share?url='.get_the_permalink().'&amp;text='.str_replace(array(' ', '  '), '%20', wp_trim_words( get_the_content(), 16, '...')).'">';
            ...
            case 'in':
                ...
                if($this->linkedin == 'in')
                {
                    $output .= '<li><a title="Linkedin: '.$this->social_counts('linkedin').'" href="http://linkedin.com/shareArticle?mini=true&amp;url='.get_the_permalink().'&amp;title='.str_replace(array(' ', '  '), '%20', get_the_title()).'&amp;ro=false&amp;summary='.str_replace(array(' ', '  '), '%20', wp_trim_words( get_the_content(), 25, '...')).'&amp;source='.get_the_permalink().'">';
            ...
            case 'pt':
                ...
                if($this->pinterest == 'pt')
                {
                    ...
                    $output .= '<li><a title="Pinterest: '.$this->social_counts('pinterest').'" href="http://pinterest.com/pin/create/button/?url='.get_the_permalink().'&amp;media='.$attachment[0].'&amp;description='.str_replace(array(' ', '  '), '%20', wp_trim_words( get_the_content(), 25, '...')).'">';
